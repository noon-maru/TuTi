...
import {
  ...
  setCurrentY,
  animateDrawer,
} from "redux/slice/courseDrawerSlice";
...
function* handleDrawerAnimation(action: AnimateDrawerAction) {
  ...
  const { translateY, currentY, courseContainerHeight } = yield select(
    (state: RootState) => state.courseDrawer
  );

  let toValue =
    direction === "UP"
      ? currentY - courseContainerHeight + 65 + 10
      : currentY + courseContainerHeight - 65 - 10;
...
export default function* watchCourseDrawerAnimation() {
